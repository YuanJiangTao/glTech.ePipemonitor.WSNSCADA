...
namespace PluginContract.Helper
{
    ...
    public class RedisHelper
    {
        ...
        {
            key = AddSysCustomKey(key);
            return Do(db => db.HashKeys(key).Select(func));
        }
        public long HashLeng(string key)
        {
            key = AddSysCustomKey(key);
        ...
        public async Task<long> HashLengAsync(string key)
        {
            key = AddSysCustomKey(key);
            return await Do(db => db.HashLengthAsync(key));
        }
        ...
        private T Do<T>(Func<StackExchange.Redis.IDatabase, T> func)
        {
